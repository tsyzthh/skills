...
**r35 两条补充**：① **术语先有指称**（INV-05 修订 · FACT-10）——承重陈述引入的新术语须指向可观察物，否则该陈述**不得进入分析**；事故实例：「code graph」此前无指称（AST 符号引用边？语义摘要图？），指认为「**代码里的符号引用**」后，「这一步由谁做」立刻变成可查证的问题。② **事实纪律两道闸**（ORD-54 · r36 修订）——**约定：不带出处 = 未查证**。**提出闸（严 · 约束 AI 自己）**：以事实语气说出或写下一条可证伪命题**之前必须先查证**，「我没查」不是出口；唯一放松在**结论**可为 `无法判定`（须留痕查了什么、落 `待查证·阻塞`、**不构成放行**，交人处置）。**使用闸**：拿它去支撑动作时若**承重**且无 home → **不许往下走**。**不因来源豁免**；执行请求不豁免。细则见 [assets/k-facts.md](assets/k-facts.md)，退化观测 = `EXP-21`。
