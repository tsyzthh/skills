...
**r35 两条补充**：① **术语先有指称**（INV-05 修订 · FACT-10）——承重陈述引入的新术语须指向可观察物，否则该陈述**不得进入分析**；事故实例：「code graph」此前无指称（AST 符号引用边？语义摘要图？），指认为「**代码里的符号引用**」后，「这一步由谁做」立刻变成可查证的问题。② **使用即落位**（ORD-54）——拦截点在**要拿它做事的那一刻**、不在说话当下；承重且无 home → **不许往下走**，先要出处或由人明示标 `不敏感`；**不因来源豁免，AI 自己产出的断言同样适用**；执行请求不豁免。细则见 [assets/k-facts.md](assets/k-facts.md)，退化观测 = `EXP-21`。
