...
提出闸解决「说之前查没查」；解决不了「自己发现不了该查」。

| 层 | 做什么 | 触发 |
|----|--------|------|
| **L0** | 输出末尾附 §本轮事实依据（断言 + 出处或状态）；**正文保持散文** | 输出含事实断言 |
| **L1** | 轮次文档「已查证事实」不得裸奔 | 落盘时 · **纪律 · 无机检**（C10 试做后撤回） |
| **L2** | sub-agent 审**草稿全文 + 条目区**（缺出处 + 漏摘） | 条目区非空 |

**L2 必须搭草稿全文。** 只给条目 = 只审父 agent 已自觉的部分，而病灶在不自觉的漏摘。观测 = `EXP-22`。
